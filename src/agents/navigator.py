...
class Navigator:
    ...
    def _build_graph(self, graph_dict: Dict[str, Any], graph_type: str) -> nx.DiGraph:
        ...
        for node in graph_dict.get("nodes", []):
            ...
            G.add_node(
                ...
                analysis_method="static analysis" if graph_type in ["module","lineage"] else "unknown",
                attrs=attrs
            )
        ...
        for edge in graph_dict.get("edges", []):
            ...
            G.add_edge(
                ...
                analysis_method="static analysis" if graph_type=="lineage" else "unknown",
                attrs=attrs
            )
        return G
    ...
    def search_by_attribute(self, key: str, value: Any) -> List[str]:
        return [node for node, data in self.merged.nodes(data=True)
    ...
    def blast_radius(self, node_id: str) -> int:
        return len(nx.descendants(self.merged, node_id)) if node_id in self.merged else 0
